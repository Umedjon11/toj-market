    ...
    return (
        <section className="flex flex-col gap-[2.5vh] w-full sm:w-[25%] text-[14px] sm:fixed z-30">
            ...
            <aside className="flex flex-col gap-[3.5vh] rounded-md bg-[#F7F8F9] dark:bg-[#1E2024] w-full p-[5vh_7%]">
                <Link href={"/profile"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] flex gap-3 items-center ${path == "/en/profile" || path == "/ru/profile" || path == "/tj/profile" ? "text-[#FFC845]" : ""}`}>{!isLoadingUser && (<img className="w-10 h-10 rounded-full cursor-pointer" draggable={false} src={user && user?.avatar != "/media/users_avatars/placeholder.png" ? (api + user?.avatar) : "https://media.istockphoto.com/id/1300845620/vector/user-icon-flat-isolated-on-white-background-user-symbol-vector-illustration.jpg?s=612x612&w=0&k=20&c=yBeyba0hUkh14_jgv1OKqIH0CCSWU_4ckRkAoy2p73o="} />)} {isLoadingUser && (<Skeleton className="w-10 h-10 rounded-full" />)} {!isLoadingUser ? (user?.first_name + " " + user?.last_name) : (<Skeleton className="w-[60%] rounded-md h-[4vh]" />)} <ChevronRight /> </Link>
                <Link href={"/profile/cart"} className={`font-semibold mt-[1vh] sm:mt-[3vh] transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><ShoppingCart className={`transition-all duration-500 ${path.includes("/profile/cart") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text1")}</Link>
                <Link href={"/profile/wish"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><Heart className={`transition-all duration-500 ${path.includes("/profile/wish") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text2")}</Link>
                <Link href={"/profile/comments"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><MessageCircle className={`transition-all duration-500 ${path.includes("/profile/comments") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text3")}</Link>
                <Link href={"/profile/myorders"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><ShoppingBag className={`transition-all duration-500 ${path.includes("/profile/myorders") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text6")}</Link>
                {user?.role == "BY" ? (
                    ...
                    <Link href={"/profile/myshop"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><Store className={`transition-all duration-500 ${path.includes("/profile/myshop") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text5")}</Link>
                ) : user?.role == "AD" ? (
                    <Link href={"/profile/categories"} className={`font-semibold transition-all duration-500 hover:text-[#FFC845] hover:*:text-[#FFC845] flex gap-3 items-center`}><ChartBarStacked className={`transition-all duration-500 ${path.includes("/profile/categories") ? "text-[#FFC845]" : "text-[#D1D3D4] dark:text-white"}`} /> {t("text20")}</Link>
                ) : null}
            </aside>
            <button onClick={() => {
                removeTokens()
