      ...
      )}</p>
      {
        wish.map((product: IProduct) => {
          return <div key={product.id} className="w-full sm:w-[30.5%] flex flex-col gap-[1vh] rounded-xl p-[1vh]">
            ...
            <p className={`${+product.avg_crowns > 0 ? "text-[#FFC845]" : "text-[#D1D3D4]"} font-semibold flex`}>{+product.avg_crowns > 0 ? (<span className="flex gap-2 items-center"><Crown /> {product.avg_crowns}</span>) : (<span className="flex gap-2 items-center"><Crown />{tt("text3")}</span>)}</p>
